            // Hamburger menu logic for mobile (for all pages)
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function showTab(tabId, updateHash = true) {
    ...
    if (tab) {
      ...
      // Optionally set the corresponding tab button as active
    ...
    // Prevent scroll jump on initial load with hash
    ...
    if (initialHash && document.getElementById(initialHash.substring(1))) {
      ...
      showTab(initialHash.substring(1), false);
      // Restore hash after a short delay (so tab logic runs first)
      setTimeout(function() {
        history.replaceState(null, '', '#' + initialHash.substring(1));
      }, 10);
    } else {
      showTab('sip_tab', false);
    }
    window.addEventListener('hashchange', openTabFromHash);

    // Attach click handler to all tab buttons to update hash
    document.querySelectorAll('.tab-btn').forEach(function(btn) {
